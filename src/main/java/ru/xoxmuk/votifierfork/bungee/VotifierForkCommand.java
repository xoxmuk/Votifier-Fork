...
public class VotifierForkCommand extends Command {
	private VotifierForkBungee bungee;

	public VotifierForkCommand(VotifierForkBungee bungee) {
		super("VotifierForkbungee", "VotifierFork.admin");
		this.bungee = bungee;
	}

	@Override
	public void execute(CommandSender sender, String[] args) {
		if (sender.hasPermission("VotifierFork.admin")) {
			if (args.length > 0) {
				if (args[0].equalsIgnoreCase("reload")) {
					bungee.reload();
					sender.sendMessage(new TextComponent("Reloading VotifierFork"));
				}
