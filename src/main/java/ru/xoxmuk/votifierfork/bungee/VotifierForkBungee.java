...
public class VotifierForkBungee extends Plugin {
	private VotifierForkBungee instance;
	@Getter
	...
	public void onEnable() {
		...
		config.load();
		loadTokens();
		getProxy().getPluginManager().registerCommand(this, new VotifierForkCommand(this));
			...
			voteReceiver = new VoteReceiver(config.getHost(), config.getPort()) {
				...
				public void callEvent(Vote vote) {
					getProxy().getPluginManager()
							.callEvent(new ru.xoxmuk.votifierfork.bungee.events.VotifierEvent(vote));
				}
