...
public class VotifierForkVelocityCommand implements SimpleCommand {
	private VotifierForkVelocity plugin;

	public VotifierForkVelocityCommand(VotifierForkVelocity plugin) {
		this.plugin = plugin;
	}
		...
		if (hasPermission(invocation)) {
			if (args.length > 0) {
				if (args[0].equalsIgnoreCase("reload")) {
					plugin.reload();
					source.sendMessage(Component.text("Reloading VotifierFork").color(NamedTextColor.AQUA));
				}
	...
	public boolean hasPermission(final Invocation invocation) {
		return invocation.source().hasPermission("VotifierFork.admin");
	}
}
