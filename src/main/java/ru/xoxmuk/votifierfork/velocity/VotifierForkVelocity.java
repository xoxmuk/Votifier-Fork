package ru.xoxmuk.votifierfork.velocity;
...
import ru.xoxmuk.votifierfork.ForwardServer;
import ru.xoxmuk.votifierfork.crypto.RSAIO;
import ru.xoxmuk.votifierfork.crypto.RSAKeygen;
import ru.xoxmuk.votifierfork.crypto.TokenUtil;
import ru.xoxmuk.votifierfork.model.Vote;
import ru.xoxmuk.votifierfork.net.ThrottleConfig;
import ru.xoxmuk.votifierfork.net.VoteReceiver;

import lombok.Getter;
import lombok.Setter;

@Plugin(id = "votifierfork", name = "VotifierFork", version = "1.0", url = "https://www.spigotmc.org/resources/VotifierFork.74040", description = "Votifier Velocity Version", authors = {
		"BenCodez" })
public class VotifierForkVelocity {
	@Getter
	...
	@Inject
	public VotifierForkVelocity(ProxyServer server, Logger logger, Metrics.Factory metricsFactory,
	...
	public void onProxyInitialization(ProxyInitializeEvent event) {
		...
		if (!configFile.exists()) {
			...
			InputStream toCopyStream = VotifierForkVelocity.class.getClassLoader()
		...
		CommandMeta meta = server.getCommandManager().metaBuilder("VotifierForkbungee")

				.aliases("VotifierFork", "VotifierForkvelocity").build();
		server.getCommandManager().register(meta, new VotifierForkVelocityCommand(this));
		loadVoteReceiver();
		...
		try {
			voteReceiver = new VoteReceiver(config.getHost(), config.getPort()) {
				...
				public void callEvent(Vote vote) {
					server.getEventManager().fire(new ru.xoxmuk.votifierfork.velocity.event.VotifierEvent(vote));
				}
